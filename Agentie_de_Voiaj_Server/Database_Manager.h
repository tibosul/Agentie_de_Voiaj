...
namespace Database
{
	...
	struct Query_Result
	{
		Result_Type type;
		std::string message;
		std::vector<std::map<std::string, std::string>> data; // Each map represents a row with column names as keys
		int affected_rows = 0; // Number of rows affected by the query (for INSERT, UPDATE, DELETE)
		
		Query_Result(Result_Type t = Result_Type::SUCCESS, const std::string& msg = "")
			: type(t), message(msg), affected_rows(0) {}

		bool is_success() const { return type == Result_Type::SUCCESS; }
	...
	private:
		...
		bool is_connected;
		std::mutex db_mutex; // Thread safety for database operations
